...
const firebaseConfig = {
    apiKey: "AIzaSyDbYKoKIcK1xIhxQyMcBDJ_tv3ZsVy7GJI",
    authDomain: "my-comment-system2.firebaseapp.com",
    projectId: "my-comment-system2",
    storageBucket: "my-comment-system2.firebasestorage.app",
    messagingSenderId: "228901896732",
    appId: "1:228901896732:web:aefdcd31945929d77715d9"
  };
  ...
  const sortedRef = query(commentsRef, orderByChild("timestamp"), limitToLast(100));

  onChildAdded(sortedRef, snapshot => {
    const comment = snapshot.val();
    renderComment(snapshot.key, comment);
  });

  window.postComment = function(parentID = null) {
    const nameField = parentID ? document.getElementById(`replyName-${parentID}`) : document.getElementById("commentName");
    const textField = parentID ? document.getElementById(`replyText-${parentID}`) : document.getElementById("commentText");
    ...
    const data = { name, text, parent: parentID || "root", timestamp: Date.now() };
  ...
  window.renderComment = function(id, data) {
    const list = document.getElementById("commentList");
    if (data.parent !== "root") {
      const parentDiv = document.getElementById(`comment-${data.parent}`);
      if (!parentDiv) return;
      const replyContainer = parentDiv.querySelector(".reply-container");
      const reply = document.createElement("div");
      reply.className = "reply";
      reply.innerHTML = `<div class='name'>${data.name}</div><div>${data.text}</div><div class='timestamp'>${new Date(data.timestamp).toLocaleString()}</div>`;
    ...
    div.id = `comment-${id}`;
    div.innerHTML = `<div class='name'>${data.name}</div><div>${data.text}</div><div class='timestamp'>${new Date(data.timestamp).toLocaleString()}</div><button class='reply-btn' onclick='showReplyBox("${id}")'>Reply</button><div class='reply-box' id='replyBox-${id}' style='display:none;'><input type='text' id='replyName-${id}' placeholder='Your name'><textarea id='replyText-${id}' placeholder='Write a reply...'></textarea><button onclick='postComment("${id}")'>Post Reply</button></div><div class='reply-container'></div>`;
